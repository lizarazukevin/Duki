...
from backend.constants import DEFAULT_TRANSCRIPTION_MODEL, OPENAI_API_BASE_URL
...
class OpenAIVoiceAdapter(VoiceAdapter):
    """Stream bounded audio into OpenAI's multipart transcription API."""
    ...
    def __init__(
        ...
        if not _MODEL_PATTERN.fullmatch(model):
            raise ValueError("OpenAI transcription model identifier is invalid")
        self._http_client = http_client
        self._api_key = api_key
        self._model = model
        self._transcriptions_url = f"{OPENAI_API_BASE_URL}/audio/transcriptions"
    ...
    async def transcribe(
        ...
        if response.status_code in {401, 403}:
            raise OpenAIConfigurationError("OpenAI transcription is not authorized")
