...
add constraint duck_sessions_resolution_suggestions_array
    check (jsonb_typeof(resolution_suggestions) = 'array'),
add constraint duck_sessions_resolution_suggestions_limit
    check (jsonb_array_length(resolution_suggestions) <= 100);

revoke all on function public.complete_duck_session(
    uuid, uuid, text, uuid, timestamptz, jsonb
) from public, anon, authenticated;

drop function public.complete_duck_session(
    uuid, uuid, text, uuid, timestamptz, jsonb
);

create function public.complete_duck_session(
    p_session_id uuid,
    p_user_id uuid,
    p_transcript text,
    p_root_task_id uuid,
...
revoke all on function public.complete_duck_session(
    uuid, uuid, text, uuid, timestamptz, jsonb, jsonb
) from public, anon, authenticated;
